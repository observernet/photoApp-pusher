...
int main(int argc, char** argv)
{
    ...
    /* 파일을 연다 */
    sprintf(filename, "%s/%s/v1.%08d.que", mdb->program_home, DATA_PATH, mdb->system_date);
	while ( 1 )
	{
...
void InitServer()
{
    ...
    /* Process를 등록한다. */
	if ( (process_id = RegistProcess(_PROC_PUSH_PROCESS_V1_)) == -1 )
	{
...
{
	/* 프로세스 등록 해제 */
	RemoveProcess(_PROC_PUSH_PROCESS_V1_);
